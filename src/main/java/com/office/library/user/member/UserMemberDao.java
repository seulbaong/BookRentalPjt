...
public class UserMemberDao {
	...
	public UserMemberVo selectUser(UserMemberVo userMemberVo) {
		...
		try {
			...
			//입력한 id를 DB에서 찾아 비밀번호를 검증 이때 비밀번호 복호화를 해야함!
			if(!passwordEncoder.matches(userMemberVo.getU_m_pw(), userMemberVos.get(0).getU_m_pw())) {
				userMemberVos.clear();
				//비밀번호를 받아왔는데 매치가 안된다면 userMemberVos 클리어(리턴하면 안되기때문)
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
		//찾아지면 0번째 리스트를 리턴(어차피 id는 중복검사를 하기때문에 리스트 0번 배열에 저장되어있음)
